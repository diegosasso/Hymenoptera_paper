...
lambda_sum <- mcmc_div %>%
  ...
  pivot_longer(everything(), names_to = "bin", values_to = "lambda") %>%
  mutate(bin = as.integer(str_extract(bin, "\\d+"))) %>%
  group_by(bin) %>%
  summarise(
    mean = mean(lambda, na.rm = TRUE),
    lower  = quantile(lambda, 0.25, na.rm = TRUE),
    upper  = quantile(lambda, 0.75, na.rm = TRUE),
    .groups = "drop"
  )
```
...
mu_sum <- mcmc_div %>%
  ...
  pivot_longer(everything(), names_to = "bin", values_to = "mu") %>%
  mutate(bin = as.integer(str_extract(bin, "\\d+"))) %>%
  group_by(bin) %>%
  summarise(
    mean = mean(mu, na.rm = TRUE),
    lower  = quantile(mu, 0.25, na.rm = TRUE),
    upper  = quantile(mu, 0.75, na.rm = TRUE),
    .groups = "drop"
  )
```
...
saveRDS(mu_sum, file='data_out/pyrate-mu.RDS')
saveRDS(lambda_sum, file='data_out/pyrate-lambda.RDS')
```
...
lambda_sum <- readRDS(file='data_out/pyrate-lambda.RDS')
mu_sum <- readRDS(file='data_out/pyrate-mu.RDS')
```
...
## Plot origination (lambda) and extinction (mu) 
...
lambda_step <- make_steps(lambda_sum$mean[1:14], edges)
mu_step     <- make_steps(mu_sum$mean[1:14], edges)

rate_df <- bind_rows(
  data.frame(
    x = lambda_step$x_step,
    y = lambda_step$y_step,
    rate = "origination rate"
  ),
  data.frame(
    x = mu_step$x_step,
    y = mu_step$y_step,
...
p <- ggplot(rate_df, aes(x = x, y = y, colour = rate)) +
  geom_line(linewidth = 1.2) +
  # geom_vline(
  #   xintercept = seq(0, 250, by = 50),
  #   colour = "grey85",
  #   linewidth = 0.35
  # ) +
  scale_x_reverse(
  ...
  scale_colour_manual(
    values = c(
      "origination rate" = "#1f78b4",
      "extinction rate"  = "#ff0000"
    )
  ) +
  labs(
    x = "Time (Ma)",
...
ggsave("figures/lambda-mu-rate.pdf", p, width = 6, height = 3.2, dpi = 300, units = "in")
```

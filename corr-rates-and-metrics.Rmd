...
p <- ggscatter(
  ...
  xlab = "",
  ylab = "net diversification rate (PyRate)"

) +
  labs(x = expression(N[eff]^"+")) +
  stat_cor(
    method = "pearson")

p
ggsave(
  "figures/metrics_thru_time/pyrate_vs_neff.pdf",
  plot = p,
  width = 3.5,
  height = 3.2
...
# cut times into bins (right=FALSE so [lower,upper) )
neff_975$bin <- cut(neff_975$time, breaks=bins, right=FALSE)

# average per bin
avg_neff_975 <- aggregate(mean_entropy ~ bin, data=neff_975, FUN=mean)
...
# cut times into bins (right=FALSE so [lower,upper) )
neff_925$bin <- cut(neff_925$time, breaks=bins, right=FALSE)

# average per bin
avg_neff_925 <- aggregate(mean_entropy ~ bin, data=neff_925, FUN=mean)
...
```{r}
# get metrics and remove last bin
neff_r_975 <- rev(avg_neff_975[,2])[-1]
neff_r_925 <- rev(avg_neff_925[,2])[-1]
```
...
plot(neff_r_975, py_div)     
model <- lm(py_div ~ neff_r_975)       
abline(model, col = "red", lwd = 2) 

cor.test(neff_r_975, py_div, method = 'pearson')
...
plot(neff_r_925, py_div)     
model <- lm(py_div ~ neff_r_925)       
abline(model, col = "red", lwd = 2) 

cor.test(neff_r_925, py_div, method = 'pearson')
...
m2 <- lm(py_div ~ log_f_plus_r + H_plus_r, data = df)
summary(m2)

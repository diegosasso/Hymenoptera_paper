---
title: "Make Pc for enrichment strength"
date: "`r Sys.Date()`"
output: 
  html_document:
    df_print: paged
...
## Calculate Pc for each replicate
...
```{r}
saveRDS(summary_s_df, file='data_out/pc-per-replicate.RDS')
```
...
```{r}
summary_s_df <- readRDS(file='data_out/pc-per-replicate.RDS')
```
...
p
ggsave("figures/metrics_thru_time/Pc-metric.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")
```

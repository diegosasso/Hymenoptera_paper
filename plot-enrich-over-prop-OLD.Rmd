...
```{r}
tip.rate.tot <- get_tip_enrichment(tree, rates_norm)
tip.prop <- tip.enr/tip.rate.tot
tip.enr <-tip.prop
```
...
nodelabels(cex=0.5)
tips_apo <- phangorn::Descendants(tree, 84, type = c("tips"))[[1]]
# enr
tip.prop.apo <- tip.prop[tips_apo,]
```

##### Stat
```{r}
mean_apo <- colMeans(tip.prop.apo)
mean_apo
...
quantile(as.numeric(tip.prop.apo), probs = c(0.025, 0.975), na.rm = TRUE)
```
##### Plot
...
file_base <- "figures/tree-enrich/apocrita-prop"
...
# Foreground bars (actual proportions)
barplot(
  vals,
 col = "#e31a1c",
  #col = "red",
...
```{r}
typs_sym <- 1:77
typs_sym <-typs_sym[-tips_apo]
tip.prop.sym <- tip.prop[typs_sym,]
```

##### Stat

```{r}
colMeans(tip.prop.sym)

mean(tip.prop.sym)

sd(tip.prop.sym)

quantile(as.numeric(tip.prop.sym), probs = c(0.025, 0.975), na.rm = TRUE)
```
...
file_base <- "figures/tree-enrich/heat_map-over-legend-prop"
...
# ---- Save to EPS ----
#if (!dir.exists("plots")) dir.create("plots")
file_base <- "figures/tree-enrich/tree_enrich-prop"

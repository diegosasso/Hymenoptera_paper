---
title: "Ontophylo: Base analyses and quality control"
date: "`r Sys.Date()`"
output: 
  html_document:
    df_print: paged
...
```{r}
# Quick quality control of model fitting: check if any estimated rates were extremely high (e.g., > 1).
...
```{r}
# Quick quality control of model fitting: check if any estimated rates were suspiciously high (e.g., > 1).
...
# Pre-organize data: (complex) dependent characters.
...
i = 1
# Fit all models for (complex) dependent characters.
...
save(hym_tree, hym_data, hym_adult_final, hym_annot_final, fitted_models_final, 
     n_stm, res, file = "data_out/paramo_stm_adult.RDA")
```
...
# Get threshold for outliers based on Q matrix estimates (thres = 0.72).
thres <- median(sapply(fitted_models_final, function(x) -mean(diag(x$Q_matrix)) ))*1000

# Get outliers based on Q matrix estimates.
drop_chars_out <- names(which(sapply(fitted_models_final, function(x) -mean(diag(x$Q_matrix)) ) > thres))
...
# Use the code below to save a single pdf (comment/uncomment if necessary).
...
# Calculate mean rates for every character (across all stochastic maps).
...
for (i in 1:length(fitted_models_larval)) {
  ...
  # Model fitting (with corHMM).
  fitted_models_larval[[i]] <- fit_models(tree = hym_tree, ch = mat[[i]], root_p = "maddfitz")
...
```{r}
# Model fitting (with corHMM).
fitted_models_larval_dep <- fit_models(tree = hym_tree, ch = char_merg, root_p = "maddfitz", model = Q, dep_char = T)
...
```{r}
# Model fitting (with corHMM).
fitted_models_larval_dep_c <- fit_models(tree = hym_tree, ch = char_merg, root_p = "maddfitz", model = Q, dep_char = T)
...
# Save data set.
save(hym_tree, hym_data, hym_larval_final, fitted_models_larval_final, n_stm, res, file = "data_out/paramo_stm_larval.RDA")
```
...
# Get threshold for outliers based on Q matrix estimates.
thres <- median(sapply(fitted_models_larval_final, function(x) -mean(diag(x$Q_matrix)) ))*100

# Get outliers based on Q matrix estimates.
drop_chars_out_lv <- names(which(sapply(fitted_models_larval_final, function(x) -mean(diag(x$Q_matrix)) ) > thres))
...
```{r}
# Plot one sample of stochastic map per character (larval) (comment/uncomment if necessary).
...
# Check for outliers: given the small sample size, it is possible to check all maps individually (see pdf above). CH11 and CH1_CH2 removed.
```

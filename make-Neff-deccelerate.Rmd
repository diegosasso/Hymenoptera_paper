---
title: "Make Neff statistics for Deccelerating Branches"
date: "`r Sys.Date()`"
output: 
  html_document:
    df_print: paged
...
# Calculate Neff for each replicate (deccelarating branches)

May take a while

```{r}
n_eff_dec_list <- map(replicate_list, function(mat) {
  enr <- get_enrichment_mask(mat, lower_q = 0.05, upper_q = 0.95,
                             direction = "under", return_type = "binary")
  out <- plot_enrichment_entropy(tree, enr, type = "Neff_scaled",
                                 n_points = 1000)$data
  out
})

n_eff_dec_df <- bind_rows(n_eff_dec_list, .id = "replicate")
head(n_eff_dec_df)
```
...
summary_neff_under_df <- n_eff_dec_df %>%
  group_by(time) %>%
  summarise(
    mean_neff = mean(entropy, na.rm = TRUE),
    lower = quantile(entropy, 0.025, na.rm = TRUE),
    median = quantile(entropy, 0.5, na.rm = TRUE),
    upper = quantile(entropy, 0.975, na.rm = TRUE),
  )
head(summary_neff_under_df)
tail(summary_neff_under_df)
```
...
```{r}
saveRDS(summary_neff_under_df, file='data_out/neff-under-per-replicate.RDS')
```
...
```{r}
p <- ggplot() +
  geom_ribbon(data = summary_neff_under_df, aes(x = time, ymin = lower, ymax = upper),
              fill = "#1f78b4", alpha = 0.2) +
  scale_x_reverse(breaks = seq(0, 280, by = 50)) +
  geom_line(data = summary_neff_under_df, aes(x = time, y = mean_neff),
            color = "#1f78b4", size = 1) +
  theme_minimal()

p
ggsave("figures/metrics_thru_time/neff-under-time.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")
```

...
ggsave("figures/metrics_thru_time/neff-pyrate-bin-50.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")


# ggsave("figures/metrics_thru_time/neff-pyrate-bin-20.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")

```

# Stepping Plot Publication

### Make average Neff

```{r}
# bin edges
bins <- c(280, seq(260, 0, by = -20) )  
length(bins)
...
edges2 <- c(280,  260, 240, 220, 200, 180, 160, 140, 120, 100, 80, 60, 40, 20, 0)
neff_step <- make_steps(neff_r, edges2)
...
df <- bind_rows(
  ...
  data.frame(
    x = py_div_step$x_step,
    y = py_div_step$y_step * 17 + 0.5,
    curve = "PyRate"
  )
)
...
p <- ggplot(df, aes(x, y, colour = curve, linetype = curve)) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      trans = ~ (. - 0.5) / 17,
      name = "net diversification rate (PyRate)"
    )
  ) +
...
ggsave("figures/metrics_thru_time/neff-pyrate-steps.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")

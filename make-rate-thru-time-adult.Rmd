...
p
#ggsave("figures/metrics_thru_time/rate-thru-time.pdf", p, width = 3.5*2, height = 1.6*2, dpi = 300, units = "in")
```
...
# BAMM
# reading func to get BAMM div rates, see make-BAMM/make-BAMM-data.R
bamm_fun <- readRDS('data/bamm_fun.rds')
# bamm_fun(1)
...
### Select unique Se, Dispr, Neff, Bamm
...
#x <- (rates_unique$mean + 1e-7) %>% asinh

ba <- asinh(bamm)
#ba <- sign(bamm) * log(abs(bamm))

#x <- (rates_unique$mean + 1e-7) %>% scale
#ba <- scale(bamm)
...
```{r}
#ne_ba <- (neff_uni$mean_entropy + 1e-7) %>% scale
#ne_ba <- (neff_uni$mean_entropy + 1e-7) %>% asinh

ne_ba <- (neff_uni$mean_entropy + 1e-7) %>% log

ba <- bamm %>% asinh
#ba <- (bamm) %>% log
#ba <- sign(bamm) * (abs(bamm))^(1/3)
#ba <- sign(bamm) * log(abs(bamm))
...
#### Pval
```{r}
cor.test(ne_ba , ba,  method = 'pearson')
```
#### Plot
...
cor.test(res_ba, res_ne, method = "pearson")
```

...
```{r}
ggplot() +
  geom_ribbon(data = summary_df, aes(x = time, ymin = lower_entropy, ymax = upper_entropy),
              fill = "blue", alpha = 0.2) +
  scale_x_reverse(breaks = seq(0, 280, by = 50)) +
  geom_line(data = summary_df, aes(x = time, y = median_entropy),
            color = "black", size = 1) +
  theme_minimal()
```
...
```{r}
ggplot() +
  geom_ribbon(data = summary_df, aes(x = time, ymin = lower_f_plus, ymax = upper_f_plus),
              fill = "blue", alpha = 0.2) +
  scale_x_reverse(breaks = seq(0, 280, by = 50)) +
  geom_line(data = summary_df, aes(x = time, y = mean_f_plus),
            color = "black", size = 1) +
  theme_minimal()
```

# Plot Shannon entropy H

```{r}
ggplot() +
  geom_ribbon(data = summary_df, aes(x = time, ymin = lower_H_plus, ymax = upper_H_plus),
              fill = "blue", alpha = 0.2) +
  scale_x_reverse(breaks = seq(0, 280, by = 50)) +
  geom_line(data = summary_df, aes(x = time, y = mean_H_plus),
            color = "black", size = 1) +
  theme_minimal()
```
